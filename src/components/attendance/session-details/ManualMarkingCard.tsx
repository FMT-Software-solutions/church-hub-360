...
export function ManualMarkingCard({
  ...
  const filteredRecords = useMemo(() => {
    ...
    return sortedRecords.filter((r) => {
      ...
      const pool: string[] = [];
      if (m) {
        pool.push((m.full_name || `${m.first_name || ''} ${m.last_name || ''}`).toLowerCase());
      ...
      if (r.marked_at) pool.push(new Date(r.marked_at).toLocaleString().toLowerCase());
    ...
    <Card>
      <CardHeader className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center gap-3">
          <Tabs value={tab} onValueChange={(v) => setTab(v as 'mark' | 'records')}>
            <TabsList>
          ...
          <TabsContent value="records">
                  ...
                  {pagedRecords.map((r) => {
                    ...
                    return (
                      <TableRow key={r.id}>
                        <TableCell>
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <div className="text-xs text-muted-foreground">{m?.branch_name || ''}</div>
                            </div>
                          </div>
                        </TableCell>
                        ...
                        <TableCell>
                          {r.is_valid ? (
                            <Badge className="text-xs bg-green-100 text-green-800 dark:bg-green-900/20 dark:text-green-300">Valid</Badge>
                          ) : (
                            <Badge className="text-xs bg-yellow-100 text-yellow-800 dark:bg-yellow-900/20 dark:text-yellow-300">Pending</Badge>
                          )}
                        </TableCell>
                      </TableRow>
                    );
            ...
            <div className="mt-4">
              <Pagination
                ...
                totalPages={Math.max(1, Math.ceil(recordTotal / recordPageSize))}
