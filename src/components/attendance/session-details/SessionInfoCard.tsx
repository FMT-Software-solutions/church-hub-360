...
import { Calendar, Clock, MapPin, Tag, Users, CheckSquare, ChevronDown, ChevronUp } from 'lucide-react';
...
export function SessionInfoCard({
  ...
  return (
    <div className='relative'>
      ...
      <Card>
        <CardHeader className="cursor-pointer hover:bg-muted/30 transition-colors" onClick={() => setIsExpanded(!isExpanded)}>
          <div className="flex items-center justify-between">
            ...
            <Button variant="ghost" size="sm" className="h-8 px-2">
              {isExpanded ? <> Show less <ChevronUp className="h-5 w-5" /></> : <> Show more <ChevronDown className="h-5 w-5" /></>}
            </Button>
          </div>
          {isExpanded && <Separator className="mt-2" />}
        </CardHeader>
