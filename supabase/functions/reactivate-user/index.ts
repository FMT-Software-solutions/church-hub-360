...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Prevent users from reactivating themselves
    if (user.id === userId) {
      return new Response(
        JSON.stringify({ error: 'Cannot reactivate yourself' }),
        {
          status: 400,
          headers: corsHeaders,
        }
    ...
    // Check if current user has permission to reactivate users in this organization
    ...
    if (!hasPermission) {
      return new Response(
        JSON.stringify({ error: 'Insufficient permissions to reactivate users in this organization' }),
        {
          status: 403,
          headers: corsHeaders,
        }
    ...
    // Reactivate user in the specific organization
    ...
    if (reactivateError) {
      return new Response(
        JSON.stringify({ error: 'Failed to reactivate user in organization' }),
        {
          status: 500,
          headers: corsHeaders,
        }
