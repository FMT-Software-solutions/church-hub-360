...
serve(async (req) => {
  ...
  try {
    ...
    const { error: authUsersError } = await supabaseAdmin
      .from('auth_users')
      .upsert({ id: userId, email: userDetails.email, is_first_login: true, password_updated: false, has_purchased: true, is_owner: true }, { onConflict: 'id' })
    ...
    const { error: userOrgError } = await supabaseAdmin
      ...
      .upsert({ user_id: userId, organization_id: org.id, role: 'owner', is_active: true, can_create_users: true }, { onConflict: 'user_id,organization_id' })
